...
export class DealerController extends ParticipantController<DealerModel> {
    ...
    protected onStand() {
        ...
        if (!this.roundController.roundModel.getResult().main) {
            this.roundController.roundModel.setResult(playerResult, 'player');
        }

        if (splitResult && !this.roundController.roundModel.getResult().split) {
            this.roundController.roundModel.setResult(splitResult, 'split');
        }
    ...
    private onBust() {
        if (!this.roundController.roundModel.getResult().main) {
            this.roundController.roundModel.setResult('dealerBust', 'player');
        }

        if (this.roundController.roundModel.splitHand && !this.roundController.roundModel.getResult().split) {
            this.roundController.roundModel.setResult('dealerBust', 'split');
        }
